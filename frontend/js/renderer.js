...
const Renderer = {
  ...
  async _renderPetList() {
    ...
    for (const p of pagedPets) {
      ...
      const img = p.image ? `<img class="card-img" src="${Utils.esc(p.image)}" alt="${Utils.esc(p.name)}" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="card-img-fallback" style="display:none;width:100%;height:120px;background:${Utils.imgColor(p.name)};border-radius:8px;align-items:center;justify-content:center;font-size:32px;color:rgba(255,255,255,0.6);font-weight:700">${Utils.esc((p.name||'?')[0])}</div>` : '';
